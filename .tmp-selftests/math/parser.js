...
const IMPLICIT_FUNCTION_PATTERN = new RegExp(`\\b(${IMPLICIT_UNARY_FUNCTIONS.join("|")})\\s*(pi|e|x|y)\\b`, "gi");
const INVERSE_FUNCTION_TEXT_LABELS = {
    asin: "sin\u207b\u00b9",
    acos: "cos\u207b\u00b9",
    atan: "tan\u207b\u00b9",
    asec: "sec\u207b\u00b9",
...
function rewriteImplicitFunctionCalls(value) {
    ...
    for (let iteration = 0; iteration < 3; iteration += 1) {
        const next = previous.replace(IMPLICIT_FUNCTION_PATTERN, (_match, fn, argument) => `${fn}(${argument})`);
        if (next === previous) {
            return next;
        }
        previous = next;
...
function hasSymbol(normalized, symbol) {
    return new RegExp(`\\b${symbol}\\b`).test(normalized);
}
function suggestOrientationMessage(orientation, normalized) {
...
function parseRenderNode(expression) {
    try {
...
function isSimpleFunctionArgument(node) {
...
function wrapBinaryTexOperand(node) {
    if (!isOperatorNode(node) || node.args.length <= 1) {
...
function renderExponentText(node) {
    const rendered = renderTextNode(node);
    return /^[0-9+-]+$/.test(rendered) ? toSuperscript(rendered) : `^(${rendered})`;
}
function renderSymbolText(name) {
    if (name === "pi") {
        return "\u03c0";
    }
    return name;
}
function renderSymbolTex(name) {
    if (name === "pi") {
        return "\\pi";
    }
    return name;
}
function renderTextFunction(name, args) {
    if (name === "sqrt") {
...
function renderTexFunction(name, args) {
    if (name === "sqrt") {
...
function formatLooseExpressionText(value) {
    ...
    display = display.replace(/\^([+-]?\d+)/g, (_match, exponent) => toSuperscript(exponent));
...
export function expressionToTex(expression) {
    const parsed = parseRenderNode(expression);
...
export function compileExpression(text) {
    ...
    try {
        ...
        const message = error instanceof Error ? error.message : "\u041d\u0435 \u0443\u0434\u0430\u043b\u043e\u0441\u044c \u0440\u0430\u0437\u043e\u0431\u0440\u0430\u0442\u044c \u0432\u044b\u0440\u0430\u0436\u0435\u043d\u0438\u0435.";
        return { ok: false, normalized, orientation, error: message };
    }
}
export function evaluateCompiled(compiled, value, orientation = "yOfX") {
    try {
