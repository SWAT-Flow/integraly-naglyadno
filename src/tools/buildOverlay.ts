    ...
    if (tool.mode === "averageValue") {
      ...
      const polylines: GraphPolyline[] = averageFinite
        ...
        const points: GraphPoint[] = averageFinite
          ? [...snapshot.points, { x: b, y: averageValue, label: `среднее=${formatNumber(averageValue, 3)}`, color: COLORS.amber, radius: 4 }]
          : snapshot.points;
      ...
      return {
        ...
        explanation: averageFinite
          ? [
              "Сначала мы сравниваем две площади: под графиком и прямоугольника ширины {{tex:b-a}}.",
              "Число {{tex:f_{\\text{ср}}}} — это высота этого прямоугольника. Если функция непрерывна, то найдётся точка {{tex:c}}, где {{tex:f(c)=f_{\\text{ср}}}}, но {{tex:c}} и {{tex:f_{\\text{ср}}}} — это не одно и то же.",
