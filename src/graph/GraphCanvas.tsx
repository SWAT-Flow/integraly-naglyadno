...
function sampleRegionPolygons(
  ...
  const polygons: GraphPoint[][] = [];
  let topPoints: GraphPoint[] = [];
  let bottomPoints: GraphPoint[] = [];
  let previousTopY: number | null = null;
...
export function GraphCanvas({ expressions, overlay, defaultView, viewResetToken = 0 }: GraphCanvasProps) {
  ...
  useEffect(() => {
    ...
    const toX = (x: number) => padding.left + ((x - view.xMin) / (view.xMax - view.xMin)) * plotWidth;
    const toY = (y: number) => padding.top + ((view.yMax - y) / (view.yMax - view.yMin)) * plotHeight;
    ...
    expressions.forEach((expression) => {
      const segments = sampleFunctionSegments(
        expression,
        view,
        plotWidth,
        plotHeight,
        Math.max(220, Math.floor(plotWidth * 1.15)),
      );
  ...
  const zoomAt = (factor: number, anchorRatioX = 0.5, anchorRatioY = 0.5) => {
    setView((current) => {
  ...
  return (
    <div className="graph-card">
      <div className="graph-toolbar">
        <IconButton label="\u0423\u0432\u0435\u043b\u0438\u0447\u0438\u0442\u044c" onClick={() => zoomAt(0.82)}>
          +
        </IconButton>
        <IconButton label="\u0423\u043c\u0435\u043d\u044c\u0448\u0438\u0442\u044c" onClick={() => zoomAt(1.22)}>
          -
        </IconButton>
        <IconButton label="\u0421\u0431\u0440\u043e\u0441\u0438\u0442\u044c \u0432\u0438\u0434" onClick={() => setView(normalizedDefaultView)}>
          {"\u21ba"}
        </IconButton>
      </div>
      <div
        ref={wrapperRef}
        className={`graph-shell ${dragging ? "graph-shell-dragging" : ""}`}
        onPointerDown={(event) => {
          if (event.button !== 0) {
            return;
          }
      ...
      <div className="graph-legend">
        {expressions.length ? (
          expressions.map((expression) => (
            <div key={expression.id} className="legend-item">
              <span className="legend-swatch" style={{ backgroundColor: expression.color }} />
              <span>{expressionLabel(expression)}</span>
            </div>
          ))
        ) : (
          <span className="empty-state">
            {decodeEscapedUnicode(
              "\u0414\u043e\u0431\u0430\u0432\u044c\u0442\u0435 \u0432\u044b\u0440\u0430\u0436\u0435\u043d\u0438\u0435 \u0441\u043b\u0435\u0432\u0430, \u0447\u0442\u043e\u0431\u044b \u0443\u0432\u0438\u0434\u0435\u0442\u044c \u0433\u0440\u0430\u0444\u0438\u043a.",
            )}
          </span>
        )}
      </div>
